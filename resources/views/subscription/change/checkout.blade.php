                    ...
                    <div class="body">
                        ...
                        <div class="list-group">
                            ...
                            @if($plan->hasTrial() && !Auth::user()->subscription('main'))
                                <div class="list-group-item">
                                    <div class="row">
                                        <strong class="col-xs-4" style="margin: 0">
                                            {{__('Trial')}}
                                        </strong>
                                        <div class="col-xs-3" style="margin: 0">
                                            {{$plan->trial_period_days}} {{__('days')}}
                                        </div>
                                        <div class="col-xs-5" style="margin: 0">
                                            <a href="javascript:void(0)" class="btn btn-info btn-sm btn-flat" id="begin-trial">
                                                {{__('Begin Trial')}}
                                            </a>
                                        </div>
                                    </div>
...
@push('js')
    ...
    <script>
        var Page = function(){
            ...
            var handleBeginTrialClick = function () {
                $('#begin-trial').click(function () {
                    swal({
                        title: "{{__('Are you sure?')}}",
                        text: "{{__('Make sure that you enable auto renewal to avoid stoppage of service after trial period.')}}",
                        type: "success",
                        ...
                        $.ajax({
                            url: '{{route('subscription.change.checkout.begin-trial')}}',
                            method: 'POST',
                            success: function() {
                                swal("{{__('Successful')}}", "{{__('You have started your trial period!')}}", "success");
            ...
            var handleApplyCreditClick = function () {
                $('#apply-credit').click(function () {
                    swal({
                        ...
                        $.ajax({
                            url: '{{route('subscription.change.checkout.apply-credit')}}',
                            method: 'POST',
                            success: function() {
