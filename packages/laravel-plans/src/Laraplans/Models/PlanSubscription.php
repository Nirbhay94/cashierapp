...
class PlanSubscription extends Model implements PlanSubscriptionInterface
{
    ...
    {
        if (!is_null($trial_ends_at = $this->trial_ends_at)) {
            return Carbon::now()->lt(Carbon::instance($trial_ends_at));
        }
    ...
    /**
     * Check if subscription is on strict trial.
     *
     * @return bool
     */
    public function onStrictTrial()
    {
        if (!is_null($trial_ends_at = $this->trial_ends_at)) {
            return $this->isEnded() && Carbon::now()->lt(Carbon::instance($trial_ends_at));
        }
    ...
    {
        $ends_at = Carbon::instance($this->ends_at);

        return Carbon::now()->gt($ends_at) or Carbon::now()->eq($ends_at);
    }
